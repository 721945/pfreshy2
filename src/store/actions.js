...
export default {
      ...
      if (Alert) {
        if (Alert.unread) {
          Alert.unread.push({
            ...
            data: "คำท้าคือ " + form.topic + " มูลค่า " +
              form.coin +
              " coin ",
          });
          ...
          AlertRef.update(
            {
              unread: [
                {
                  title: "คุณได้รับคำท้า",
                  data: "คำท้าคีอ " + form.topic + " มูลค่า " +
                    form.coin +
                    " coin ",
                },
              ],
      ...
      if (Alert) {
        if (Alert.unread) {
          Alert.unread.push({
            ...
            data: "คำท้าคีอ " + bet.topic + " มูลค่า " + bet.coin +
              " coin ",
          });
          AlertRef.update(Alert);
        } else {
          AlertRef.update(
            {
              unread: [
                {
                  title: "คำท้าของคุณได้รับการยอมรับแล้ว",
                  data: "คำท้าคีอ " + bet.topic + " มูลค่า " +
                    bet.coin +
                    " coin ",
                },
              ],
      ...
      if (Alert) {
        if (Alert.unread) {
          Alert.unread.push({
            ...
            data: "คำท้าคือ " + bet.topic + " มูลค่า " +
              bet.coin +
              " coin ",
          });
          AlertRef.update(Alert);
            ...
            if (Alert) {
              if (Alert.unread) {
                Alert.unread.push({
                  ...
                  data: "คำท้าคีอ " + bet.topic + " มูลค่า " +
                    bet.coin +
                    " coin " + "ผู้ชนะคือ " + state.authUser.name,
                });
                AlertRef.update(Alert);
              } else {
                AlertRef.update(
  ...
  async LoadAlert({ commit, state }) {
    try {
      const alertRef = this.$fireStore.collection("alert").doc(
        state.authUser.uid,
      );
      const alertGet = await alertRef.get();
