...
export default {
  ...
  async LoadBetSend({ commit, state }) {
    const BetRef = this.$fireStore.collection("Bet");
    try {
      const Res = await BetRef.where("player1", "==", state.authUser.uid).where(
        "accept",
        "==",
        false,
      ).get();
  ...
  async BetRefuse({ commit }, bet) {
    try {
      ...
      bet.read = false;
      const insert = await this.$fireStore.collection("BetResult").doc(
        bet.Betid,
      ).set(bet);
      ...
      if (x.winner != "") {
        ...
        if (x.loser != "") {
          if (x.loser != state.authUser.uid) {
            ...
            console.log(bet.loser, bet.winner);
            const insert = await this.$fireStore.collection("BetResult").doc(
              bet.Betid,
            ).set(bet);
